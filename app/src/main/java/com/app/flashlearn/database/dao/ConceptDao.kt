...
interface ConceptDao {
    ...
    @Query("""
        ...
        WHERE c.active = 1 AND ct.text LIKE (\x27%\x27 || :query || \x27%\x27)
    """)
    ...
    @Query("""
        SELECT c.* FROM concepts c
        JOIN learning_states ls ON ls.conceptId = c.id
        WHERE c.active = 1 AND ls.stage = \x27DAILY\x27 AND ls.totalCorrect = 0 AND ls.totalWrong = 0
    ...
    @Query("""
        ...
        WHERE c.active = 1 AND (
            ls.stage IN (\x27WEEKLY\x27, \x27MONTHLY\x27)
            OR (ls.stage = \x27DAILY\x27 AND (ls.totalCorrect > 0 OR ls.totalWrong > 0))
        )
